...
#if defined(NDEBUG)
# error "Universecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x62807fd871ac79a7a0cde3b7ef5037a642158ce661c55849abecd39148e65735");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 18); // Universecoin: starting difficulty is 1 / 2^14
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    int blockofMonth = 30*1440*2;
    int nMonth = nHeight/blockofMonth;
    nSubsidy -= (nMonth<5 ? nMonth : 5)*10*COIN;

    if(nHeight == 1)   	    nSubsidy = 8400000 * COIN;
    if(nHeight > 5875200)   nSubsidy = 0 * COIN;
...
static const int64 nTargetTimespan = 30 * 3000; // Universecoin: 3.5 days
static const int64 nTargetSpacing = 30; // Universecoin: 0.5 minutes
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "universecoin,2015-5-10";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
	printf("test for the genesisTxInBytes\n");
        printf("%s\n\n",HexStr(txNew.vin[0].scriptSig.begin(), txNew.vin[0].scriptSig.end()).c_str());
	uint256 hash = block.GetHash();
        printf("%s\n", hash.ToString().c_str());
        printf("%s\n", hashGenesisBlock.ToString().c_str());
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x9527ca8bc3f9615ce2538c15ea157a94555a798dcb8abb4f15154a2ac5f518e9"));
...
void static UniverseCoinMiner(CWallet *pwallet)
{
    printf("UniverseCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running UniverseCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UniverseCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UniverseCoinMiner, pwallet));
}
